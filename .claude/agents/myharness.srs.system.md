...
3. **Self-verification** — run the quality checklist (see below) **for CONTENT completeness only** (every module has a folder, every entity in ERD, every actor mapped, no skipped FEA). Fix content gaps. Do NOT treat IDE markdown diagnostics as gaps — see the rule below.
...
>
> **Write each file ONCE. Do NOT re-edit any file to chase markdown diagnostics.** Your self-verification is about CONTENT coverage, not a green diagnostics panel. Each file = one write; if a file legitimately needs a content fix, make that single targeted edit and move on — never a fix-recheck-fix loop.
>
