...
| Timestamp | `protocols/timestamp-protocol.md` | Before writing ANY orchestrator log entry |
| Log Formats | `protocols/log-formats.md` | When writing orchestrator log entries |
| Implement Delegation | `protocols/implement-delegation.md` | Before delegating to `myharness.implement` (Step 10) |
...
| Scope Guard | `protocols/scope-guard-protocol.md` | Before EVERY `myharness.implement` dispatch |
| Run State | `protocols/run-state-protocol.md` | At STEP 0 (create) + after every step (update) |
| Health Check | `protocols/health-check-protocol.md` | After Step 13 (pipeline complete) |
...
Write to `docs/output/run-logs/<feature-id>/00-myharness.log.md` incrementally per `protocols/timestamp-protocol.md`.
...
2. Read `protocols/pipeline-context.md` and create `run-context.yaml`
3. **STEP 0**:
...
6. **Before EVERY `myharness.implement` dispatch**: read and follow `protocols/scope-guard-protocol.md` — run scope_guard, write `[SCOPE-CHECK]` log entry, block if violations found
...
8. After Step 13: run health check per `protocols/health-check-protocol.md`, write `[HEALTH-REPORT]` log entry
9. At pipeline end:
   - Generate token report: write `docs/output/run-logs/<feature-id>/token-report.md` using `templates/token-report-template.md` — populate from accumulated `state.yaml.token_summary` and per-step `metrics` fields
   ...
   - Output completion report per `templates/pipeline-completion.md`
