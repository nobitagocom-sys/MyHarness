...
> **PRE-CHECK (MANDATORY before dispatching):** Run this detection before deciding which SRS agent to invoke. There are **THREE** branches — evaluate in order.
>
> ```text
...
> ---
>
> **BRANCH A — overview file EXISTS (pre-generated):**
>
...
> ---
>
> **BRANCH B — overview MISSING and input spec is MULTI-MODULE → dispatch `myharness.srs.system` FIRST:**
>
> Determine module count from the `--N <spec-path>` input file (CREATE mode): read the spec and count distinct modules / feature groups. Treat as **multi-module** if the spec describes **2 or more** modules, OR if it is a system/product-level requirements document (contains multiple top-level feature sections, multiple actors, or an explicit module list).
>
> If multi-module:
>
> 1. Set `srs-system.status: GENERATING` in `run-context.yaml`
> 2. **Dispatch `myharness.srs.system`** with the delegation block below. It writes `docs/output/srs-systems/srs-overview-system.md` + per-module folders `modXX-*/`.
...
> 5. Write `[STEP 1 — system SRS generated via myharness.srs.system → N modules]` in orchestrator log.
>
...
> ---
>
> **BRANCH C — overview MISSING and input is SINGLE-MODULE → dispatch `myharness.srs`:**
> Dispatch `myharness.srs` normally as below (one module only).
